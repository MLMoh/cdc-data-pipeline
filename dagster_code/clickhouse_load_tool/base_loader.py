...
class DataSourceLoader(ABC):
    ...
    def _perform_incremental_load_special(self, table_function: str, table_name: str, columns: List[str], derived_column: str = None, source: str = None) -> int:
        ...
        try:
            ...
            if self.upsert_key and self.tracking_column and self.upsert_key in columns and self.tracking_column in columns:
                self.logger.info(f"Starting deduplication: keeping latest {self.tracking_column} per {self.upsert_key}")
                ...
                if duplicate_groups > 0:
                    self.logger.info(f"Found {duplicate_groups} groups with duplicates, cleaning up...")
                ...
                self.logger.info(f"Skipping deduplication: {', '.join(missing_requirements)}")
    ...
    def get_last_loaded_value(self, table_name: str) -> Optional[str]:
        ...
        if not self.table_exists(table_name):
            self.logger.warning(f"Table {table_name} does not exist yet. Fresh load — no last value.")
            return None
    ...
    def extract_to_storage(self, source_table: str, target_table: str, destination: str = 's3', source_schema: str = None, load_type: str = 'incremental', output_key: str = None, **kwargs) -> str:
        try:
            ...
            if destination.lower() == 's3':
                ...
                self.logger.info(f"Data extracted to S3: s3://{self.s3_bucket}/{storage_key}")
            elif destination.lower() == 'gcs':
                storage_key = self.upload_to_gcs(data_generator, storage_key)
                self.logger.info(f"Data extracted to GCS: gs://{self.gcs_bucket}/{storage_key}")
        ...
        try:
            ...
            for col in all_columns:
                ...
                if len(value_types) > 1:
                    self.logger.warning(f"Column '{col}' has mixed types. Using String type.")
                    schema[col] = "String"
                    continue
            ...
            self.logger.info(f"Successfully created table {target_table}" + 
                        (f" (with derived column '{derived_column}')" if derived_column else ""))
